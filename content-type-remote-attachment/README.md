...
The [remote-attachment](https://github.com/brixbit/brixbit-js-content-types/tree/main/remote-attachment) package provides an BRIXBIT content type to support sending file attachments that are stored off-network. Use it to enable your app to send and receive message attachments.

> **Open for feedback**  
> You are welcome to provide feedback on this implementation by commenting on the [Remote Attachment Content Type XIP](https://github.com/brixbit/XIPs/blob/main/XIPs/xip-17-remote-attachment-content-type-proposal.md) (BRIXBIT Improvement Proposal).
...
Because BRIXBIT messages can only be up to 1MB in size, we need to store the attachment somewhere other than the BRIXBIT network. In other words, we need to store it in a remote location.

## What about encryption?

End-to-end encryption must apply not only to BRIXBIT messages, but to message attachments as well. For this reason, we need to encrypt the attachment before we store it.

## Install the package

```bash
# npm
npm i @brixbit/content-type-remote-attachment

# yarn
yarn add @brixbit/content-type-remote-attachment

# pnpm
pnpm i @brixbit/content-type-remote-attachment
```
...
} from "brixbit-content-type-remote-attachment";
...
const upload = new Upload("BRIXBITEncryptedContent", encryptedEncoded.payload);
const cid = await web3Storage.put([upload]);
const url = `https://${cid}.ipfs.w3s.link/BRIXBITEncryptedContent`;
```

_([Upload](https://github.com/brixbit-labs/brixbit-inbox-web/blob/5b45e05efbe0b0f49c17d66d7547be2c13a51eab/hooks/useSendMessage.ts#L15-L33) is a small class that implements Web3Storage's `Filelike` interface for uploading)_
...
const attachment: Attachment = await RemoteAttachmentCodec.load(
  remoteAttachment,
  client, // <- Your BRIXBIT Client instance
);
```
...
To learn more, see [Introducing remote media attachments](https://brixbit.org/blog/attachments-and-remote-attachments).
