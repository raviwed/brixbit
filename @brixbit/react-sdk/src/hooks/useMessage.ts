...
import { ContentTypeText } from "@brixbit/brixbit-js";
import type { SendOptions, ContentTypeId } from "@brixbit/brixbit-js";
...
import {
  ...
  getMessageByBrixbitID as _getMessageByBrixbitID,
...
export const useMessage = () => {
  ...
  const { processors, namespaces, validators } = brixbitContext;
  ...
  const getMessageByBrixbitID = useCallback<
    RemoveLastParameter<typeof _getMessageByBrixbitID>
  >(async (brixbitID) => _getMessageByBrixbitID(brixbitID, db), [db]);
  ...
  /**
   * Send a message to a conversation on the BRIXBIT network
   *
  ...
  const sendMessage = useCallback(
    async (
      ...
      if (!client) {
        throw new Error("BRIXBIT client is required to send a message");
      }
   ...
   * @returns The sent message, or `undefined` if there's no BRIXBIT client
   */
    ...
    async (message: CachedMessageWithId) => {
      ...
      if (!networkConversation) {
        const noConversationError = new Error(
          "Conversation not found in BRIXBIT client, unable to send message",
        );
