...
export const useCanMessage = (onError?: OnError["onError"]) => {
  ...
  /**
   * Check if one or more wallet addresses is on the BRIXBIT network using the
   */
  const canMessage = useCallback(
    async <T extends string | string[]>(
      ...
      if (!client) {
        throw new Error(
          "BRIXBIT client is required to check if an address is on the network",
        );
      }
  ...
  /**
   * Check if a wallet address is on the BRIXBIT network without a client instance
   */
