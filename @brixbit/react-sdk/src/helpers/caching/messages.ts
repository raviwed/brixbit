import type {
  ...
  DecodedMessage,
  SendOptions,
} from "@brixbit/brixbit-js";
import { ContentTypeText, decodeContent } from "@brixbit/brixbit-js";
 ...
 */
export const getMessageByBrixbitID = async (brixbitID: string, db: Dexie) => {
  const messages = db.table("messages") as CachedMessagesTable;
  const message = await messages.where("brixbitID").equals(brixbitID).first();
...
export const saveMessage = async (message: CachedMessage, db: Dexie) => {
  ...
  const existing = await getMessageByBrixbitID(message.brixbitID, db);
...
export const prepareMessageForSending = async ({
  ...
  if (!networkConversation) {
    throw new Error(
      "Conversation not found in BRIXBIT client, unable to prepare message",
    );
  }
  ...
  const message = {
    ...
    brixbitID: await preparedMessage.messageID(),
  } satisfies CachedMessage;
...
// BRIXBIT IDs of messages currently being processed
...
export const processMessage = async (
  ...
  if (processQueue.includes(message.brixbitID)) {
    return {
      status: "queued",
      message,
    };
  ...
  processQueue.push(message.brixbitID);

  const namespace = namespaces[message.contentType];

  try {
    const existingMessage = await getMessageByBrixbitID(message.brixbitID, db);
    ...
    const index = processQueue.indexOf(message.brixbitID);
    if (index > -1) {
